...
static int my_sem_init(void)
{
	...
	if (alloc_chrdev_region(&dev_sem , 0, 3, "SemIntDriver")<0)
	{
		pr_err("Major Num not Allocated\n");
		return -1;
	}
	pr_info("MAJOR NUMBER: %d\nMINOR NUMBER: %d\n",MAJOR(dev_sem),MINOR(dev_sem));
	
	my_dev = cdev_alloc();
	my_dev->ops = &fops;
	
	if(cdev_add(my_dev, dev_sem, 3)<0)
	{
...
static void my_sem_exit(void)
{
	...
	unregister_chrdev_region(dev_sem, 3);
...
MODULE_DESCRIPTION("SEMAPHORE INTERRUPTIBLE");
